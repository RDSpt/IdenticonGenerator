...
public class IdenticonBW {
	...
	public IdenticonBW(String name) {
		
		this.name = name;
		hash = hashInput(name);
		this.color = pickColor(hash);
		grid = buildGrid(hash);
		paintGrid = getValue(grid);
	}
	...
	private Map<Integer, Integer> getValue(List list) {
	...
	public LinkedList<Integer> getGrid() {
		return grid;
	}
